...
\begin{itemize}
\type{Coups}{\tableauUneDimension{1..60}{de}{ Coup}}
\end{itemize}
	...
	pasDiag : \entier}
	{
		...
		\pour{o}{1}{3}{2}{
		  ...
		  \tantque{((k $>$ 0) et (k $\leqslant$ 8) et (m $>$ 0) et (test $=$ FAUX) et non(estCaseVide(plateau, fixerPosition(m, k))))}{
			  ...
			  {\affecter{test}{VRAI}
			  \sialors{(k>j+1) ou (k<j-1) ou (m>i+1) ou (m<i-1)}{
				\affecter{aModifie}{VRAI}
			  }}
			  {\affecter{k}{j $+$ pas}
			  \affecter{m}{m $+$ pasDiag}}%FinSi
		  }%FinTantQue
		  \sialors{test}
		  {
			  \affecter{l}{m $-$ pasDiag}
			  \affecter{n}{k $-$ pas}
			  \repeter{
				  \instruction{inverserPion(plateau, fixerPosition(l, n))}
				  \affecter{l}{l $-$ pasDiag}
				   \affecter{n}{n $-$ pas}
			  }{(l $=$ i) et (n $=$ j)}%FinRepeter
		  } %FinSI
		}
	}
\end{algorithme}
